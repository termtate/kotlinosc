...
class OscRouterTest {
    private fun dispatcher(router: OscRouter): OscDispatcher {
        return OscDispatcher(router, OscConfig.AddressPattern.default)
    }
    ...
    fun `osc router & dispatcher basic test`() = runBlocking {
        ...
        val dispatcher = dispatcher(router)
    ...
    fun `osc dispatcher bundle all match count test`() = runBlocking {
        ...
        val hit = dispatcher(router).dispatch(bundle, DispatchMode.ALL_MATCH)
    ...
    fun `osc dispatcher bundle first match count test`() = runBlocking {
        ...
        val hit = dispatcher(router).dispatch(bundle, DispatchMode.FIRST_MATCH)
